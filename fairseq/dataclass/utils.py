...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    if args is not None:
        if hasattr(args, "task"): #如果 args 中有 task 字段，则调用 migrate_registry 函数迁移任务配置项；否则，将 task 添加到 deletes 列表中。
...
def convert_namespace_to_omegaconf(args: Namespace) -> DictConfig:
    ...
    overrides, deletes = override_module_args(args) #根据 args 中的字段动态生成配置覆盖项和删除项
    ...
    cfg = OmegaConf.create(    #处理后的参数
        OmegaConf.to_container(composed_cfg, resolve=True, enum_to_str=True)
    )

    # hack to be able to set Namespace in dict config. this should be removed when we update to newer
    # omegaconf version that supports object flags, or when we migrate all existing models
    # 为了在字典配置中设置 Namespace 的临时解决方案。当我们更新到支持对象标志的较新版 omegaconf 时，
    # 或者当我们迁移所有现有模型时，这个解决方案应该被移除。
    from omegaconf import _utils

    with omegaconf_no_object_check():
        if cfg.task is None and getattr(args, "task", None): #检查 args 中是否有 task 属性且不为 None
            cfg.task = Namespace(**vars(args))   #vars(args)：将 args 转换为字典。Namespace(**vars(args))：使用字典中的键值对创建一个新的 Namespace 对象。
...
def overwrite_args_by_name(cfg: DictConfig, overrides: Dict[str, any]): #根据 overrides 字典中的值覆盖 cfg 中的相应值
...
def merge_with_parent(dc: FairseqDataclass, cfg: DictConfig, remove_missing=False): #  将数据类 dc 与配置 cfg 合并
    if remove_missing:
