...
#             src_lengths,
#             prev_output_tokens,
#             authe_imgs_list,  # authentic feature list
#             authe_masks_list,
#             synth_imgs_list,  # synthetic feature list
#             synth_masks_list,
...
#             authe_masks_list=authe_masks_list, authe_imgs_list=authe_imgs_list,
#             synth_masks_list=synth_masks_list, synth_imgs_list=synth_imgs_list,
#         )
#         encoder_out['nmt'] = self.encoder(
#             src_tokens, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens,
#             authe_masks_list=authe_masks_list, authe_imgs_list=None,
#             synth_masks_list=synth_masks_list, synth_imgs_list=synth_imgs_list,
#         )
#         encoder_out['syn'] = self.encoder(
#             src_tokens, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens,
#             authe_masks_list=synth_masks_list, authe_imgs_list=synth_imgs_list,
#             synth_masks_list=synth_masks_list, synth_imgs_list=synth_imgs_list,
#         )
#         decoder_out = {}
...
#             return_all_hiddens=return_all_hiddens,
#         )
#         decoder_out['syn'] = self.decoder(
#             prev_output_tokens,
#             encoder_out=encoder_out['syn'],
#             features_only=features_only,
#             alignment_layer=alignment_layer,
#             alignment_heads=alignment_heads,
#             src_lengths=src_lengths,
...
#         self.selective_attns.extend([SimpleGate(vdim=i, intermediate_dim=embed_dim, output_dim=embed_dim,
#                                                 attn_drop=args.SA_attention_dropout) for i in args.image_feat_dim])
#
...
#
#         self.image_feat_dim = args.image_feat_dim[0]
#
...
#         image = image.type(text.dtype)
#         output = self.selective_attns[idx](query=text, value=image, key_padding_mask=image_mask)  # t, b, c
#
#         merge = torch.cat([output, text], dim=-1)
#         gate = self.ffw(merge)
#         gate = torch.tanh(gate)
#
#         # merge = self.ffw(merge)
#
...
#             for img, img_mask in zip(authe_imgs_list, authe_masks_list):
...
#             if authe_imgs_list is not None:
#                 for img, img_mask in zip(authe_imgs_list, authe_masks_list):
#                     img = img.transpose(0, 1)
#                     xs.append(self.fuse_img_feat(x, idx, img, img_mask, text_mask=src_tokens.ne(self.padding_idx)))
#                     idx += 1
#             else:
#                 img = torch.zeros(1, x.shape[1], self.image_feat_dim).to('cuda')
#                 xs.append(self.fuse_img_feat(x, idx, img, None, text_mask=src_tokens.ne(self.padding_idx)))
#                 idx += 1
#
#         x = self.f(xs, fun='sum')
#
...
#         )
#         new_image_feature = (
#             encoder_out.image_feature
#             if encoder_out.image_feature is not None
#             else encoder_out.image_feature.index_select(1, new_order)
#         )
...
#             image_feature=new_image_feature, # T x B x C
