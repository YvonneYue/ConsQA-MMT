...
@register_criterion(
    "label_smoothed_cross_entropy_emmt", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def add_args(parser):
        ...
        # fmt: on
        parser.add_argument('--kl-weight', action='store_true', help='weight of the kl loss')
        parser.add_argument('--ot-weight', default=0, type=int, help='weight of the ot loss')
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        encoder_output, net_output = model(**sample["net_input"])
        loss, nll_loss, loss_dict = self.compute_loss(model, net_output, sample, encoder_output, reduce=reduce)
        sample_size = (
        ...
        logging_output = {
            ...
            "mmt_loss": loss_dict["mmt"].data if 'mmt' in loss_dict else 0,
            "nmt_loss": loss_dict["nmt"].data if 'nmt' in loss_dict else 0,
            "syn_loss": loss_dict["syn"].data if 'syn' in loss_dict else 0,
            "kl_mt_loss": loss_dict["kl_mt"].data if 'kl_mt' in loss_dict else 0,
            "kl_as_loss": loss_dict["kl_as"].data if "kl_as" in loss_dict else 0,
            "ot_loss": loss_dict["ot"].data if 'ot' in loss_dict else 0,
    ...
    def compute_loss(self, model, net_output, sample, encoder_output, reduce=True):
        loss_dict = {}
        ...
        assert mmt and nmt and syn, "You need to specify both mmt and nmt"
        if mmt:
        ...
        if nmt:
            ...
            loss = (loss + loss_dict['nmt']) / 2 if loss is not None else loss_dict['nmt']
        ...
        if syn:
            ...
            loss = (loss + loss_dict['syn']) / 2 if loss is not None else loss_dict['syn']
            nll_loss = (nll_loss + loss_dict['syn_nll']) / 2 if nll_loss is not None else loss_dict['syn_nll']
        ...
        if mmt and syn:
            if self.kl_weight:
                kl_as_loss = kl_div_loss(lprobs_mmt, lprobs_syn, target)
                loss_dict['kl_as'] = kl_as_loss
                loss += self.kl_weight * kl_as_loss

        loss_dict['ot'] = compute_ot_loss(encoder_output['mmt'].image_feature, encoder_output['syn'].image_feature)
        if self.ot_weight:
            loss += self.ot_weight * loss_dict['ot']
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        kl_mt_loss_sum = sum(log.get("kl_mt_loss", 0) for log in logging_outputs)
        kl_as_loss_sum = sum(log.get("kl_as_loss", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "mmt_loss", mmt_loss_sum / ntokens / math.log(2), ntokens, round=3
        )
        metrics.log_scalar(
            "nmt_loss", nmt_loss_sum / ntokens / math.log(2), ntokens, round=3
        )
        metrics.log_scalar(
            "syn_loss", syn_loss_sum / ntokens / math.log(2), ntokens, round=3
        )
        metrics.log_scalar(
            "kl_mt_loss", kl_mt_loss_sum / ntokens / math.log(2), ntokens, round=3
        )
        metrics.log_scalar(
            "kl_as_loss", kl_as_loss_sum / ntokens / math.log(2), ntokens, round=3
        )
        metrics.log_scalar(
