...
@dataclass
class LabelSmoothedCrossEntropy_consqa_mmt_Config(FairseqDataclass):
...
@register_criterion(
    "consqa_mmt_label_smoothed_cross_entropy", dataclass=LabelSmoothedCrossEntropy_consqa_mmt_Config
)
class LabelSmoothedCrossEntropy_ConsQA_MMT_Criterion(FairseqCriterion):
    def __init__(
            self,
            task,
            sentence_avg,
    ...
    def vqa_get_src_gpt_lprobs_and_target(self, src_gpt_vqa_x, sample):
        src_gpt_lprobs = utils.log_softmax(src_gpt_vqa_x, dim=-1, onnx_trace=False)
        src_gpt_target = sample["src_gpt_ans"]
        if self.ignore_prefix_size > 0:
            # lprobs: B x T x C
            src_gpt_lprobs = src_gpt_lprobs[:, self.ignore_prefix_size:, :].contiguous()
            src_gpt_target = src_gpt_target[:, self.ignore_prefix_size:].contiguous()
        return src_gpt_lprobs.view(-1, src_gpt_lprobs.size(-1)), src_gpt_target.view(-1)
    ...
    def compute_vqa_loss(self, src_vqa_x, tgt_vqa_x, src_gpt_vqa_x, sample, reduce):
        ...
        src_gpt_lprobs, src_gpt_target = self.vqa_get_src_gpt_lprobs_and_target(src_gpt_vqa_x, sample)
        ...
        src_gpt_loss, src_gpt_nll_loss = label_smoothed_nll_loss(
            src_gpt_lprobs,
            src_gpt_target,
            self.eps,
            ignore_index=self.padding_idx,
        ...
        loss = src_loss + tgt_loss + src_gpt_loss
        nll_loss = src_nll_loss + tgt_nll_loss + src_gpt_nll_loss
...
#4
    ...
    def forward(self, model, sample, current_epoch, reduce=True):
        ...
        if self.training and sample["net_input"]["src_query"] is not None and sample["net_input"]["tgt_query"] is not None and sample["net_input"]["src_gpt_query"] is not None:
            vqa_loss, vqa_nll_loss, cons_loss = self.compute_vqa_loss(net_output['mmt'][1]["src_vqa_x"], net_output['mmt'][1]["tgt_vqa_x"], net_output['mmt'][1]["src_gpt_vqa_x"],sample, reduce=reduce)
        else:
        ...
        if self.training and sample["net_input"]["src_query"] is not None and sample["net_input"]["tgt_query"] is not None:
            logging_output = {
                ...
                "mmt_loss": mmt_loss.data if mmt_loss is not None else 0,
                "nmt_loss": nmt_loss.data,
                "kl_mt_loss": kl_loss.data if kl_loss is not None else 0,
            ...
            logging_output = {
                "loss": loss.data,
                "mt_loss": mt_loss.data if mt_loss is not None else 0,
                "mmt_loss": mmt_loss.data if mmt_loss is not None else 0,
                "nmt_loss": nmt_loss.data if nmt_loss is not None else 0,
                "kl_mt_loss": kl_loss.data if kl_loss is not None else 0,
                "nll_loss": nll_loss.data if nll_loss is not None else 0,
                "ntokens": sample["ntokens"],
                "nsentences": sample["target"].size(0),
                "sample_size": sample_size,
            }
