...
@dataclass
class LabelSmoothedCrossEntropyMMTVQA2CriterionConfig(FairseqDataclass):
...
@register_criterion(
    "label_smoothed_cross_entropy_mmt_vqa2", dataclass=LabelSmoothedCrossEntropyMMTVQA2CriterionConfig
)
class LabelSmoothedCrossEntropyMMTVQA2Criterion(FairseqCriterion):
    def __init__(
            self,
            task,
            sentence_avg,
...
#4
    ...
    def forward(self, model, sample, current_epoch, reduce=True):
        ...
        net_output = model(**sample["net_input"])  #net_output
        # print("net_output :", net_output)
        mmt_loss, nll_loss = self.compute_loss(model, net_output, sample, reduce=reduce)   #翻译损失
        if self.training and sample["net_input"]["src_query"] is not None and sample["net_input"]["tgt_query"] is not None:
            vqa_loss, vqa_nll_loss, cons_loss = self.compute_vqa_loss(net_output[1]["src_vqa_x"], net_output[1]["tgt_vqa_x"], sample, reduce=reduce)
        else:
        ...
        loss = mmt_loss + vqa_loss * self.weight1 + cons_loss * lambda_consistency
        ...
        return loss, mmt_loss, vqa_loss, cons_loss, sample_size, logging_output
