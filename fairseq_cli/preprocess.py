...
#data/consqa-m30k-en2$tgt/train.en
...
def _make_binary_dataset(
    ...
    print("full_output_prefix: {}".format(full_output_prefix))
    ...
    print("lang_index:",vocab.lang_index)
...
def main(args):
    ...
    if args.joined_dictionary:
        ...
        if args.srcdict:
            ...
            if args.task == "consqa_mmt":
                if args.pretrainpref is not None and args.prevalidpref is not None:
                    filenames = [args.trainpref + '.' + args.source_lang, args.trainpref + '.' + args.target_lang,
                                 args.srcquery + '.query', args.srcans + '.ans',
                                 args.srcgptquery + '.query',args.srcgptans + '.ans',
                                 args.tgtquery + '.query', args.tgtans + '.ans',
                                 args.pretrainpref + '.' + args.source_lang, args.pretrainpref + '.' + args.target_lang,
                                 ]
                else:
                    filenames=[args.trainpref+'.'+args.source_lang, args.trainpref+'.'+args.target_lang,
                         args.srcquery+'.query', args.srcans+'.ans', args.srcgptquery+'.query', args.srcgptans+'.ans',
                         args.tgtquery+'.query', args.tgtans+'.ans']

                print("in preprocess.py,filenames={}".format(filenames))
                src_dict = _build_dictionary(
                    filenames,  # data/consqa-m30k-en2$tgt/train.en #data/consqa-m30k-en2$tgt/train.de
                    task=task,
                    args=args,
                    src=True,
                )
                ...
                src_dict = _build_dictionary(
                    ...
                    },  # data/consqa-m30k-en2$tgt/train.en #data/consqa-m30k-en2$tgt/train.de
                    task=task,
                    args=args,
                    src=True,
                )
    ...
    if args.task == "mmt_vqa":
        _make_binary_dataset(src_dict, input_prefix=args.query, output_prefix='train', lang='query', num_workers=args.workers, args=args)
        _make_binary_dataset(src_dict, input_prefix=args.ans, output_prefix='train', lang='ans', num_workers=args.workers, args=args)

    if args.task == "mmt_vqa_2":#input: train.query_en output: train.en-de.query_en
        ...
        _make_binary_dataset(src_dict, input_prefix=args.tgtquery, output_prefix='train_de', lang='query',
    ...
    if args.task == "consqa_mmt":#train_en
        ...
        _make_binary_dataset(src_dict, input_prefix=args.srcgptquery, output_prefix='gpt_train_en', lang='query',
                             num_workers=args.workers, args=args)
        _make_binary_dataset(src_dict, input_prefix=args.srcgptans, output_prefix='gpt_train_en', lang='ans',
                             num_workers=args.workers, args=args)

        if args.pretrainpref is not None and args.prevalidpref is not None:
            _make_binary_dataset(src_dict, input_prefix=args.pretrainpref, output_prefix='pre_train', lang=args.source_lang,
                                 num_workers=args.workers, args=args)
            _make_binary_dataset(src_dict, input_prefix=args.pretrainpref, output_prefix='pre_train',
